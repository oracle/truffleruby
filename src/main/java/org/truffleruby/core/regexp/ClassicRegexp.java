...
public class ClassicRegexp implements ReOptions {
    ...
    public static Regex makeRegexp(RubyContext context, RubyDeferredWarnings rubyDeferredWarnings,
        ...
        try {
            ...
            throw new DeferredRaiseException(c -> {
                return new RaiseException(c, c.getCoreExceptions().regexpError(errorMessage, currentNode));
            });
        }
    }
    ...
    public static int raisePreprocessError(Rope str, String err, RegexpSupport.ErrorMode mode) {
        switch (mode) {
            case RAISE:
                throw new DeferredRaiseException(context -> {
                    return new RaiseException(context, context.getCoreExceptions().regexpError(err, null));
                });
            case PREPROCESS:
                throw new DeferredRaiseException(context -> {
    ...
    public static void preprocessCheck(Rope bytes) {
