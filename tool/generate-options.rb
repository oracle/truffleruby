  ...
  stabilities.each do |stability, category_options|
    category_options.each do |constant, values|
      ...
      when /^enum\/(\w*)/
        ...
        option_type = "EnumOptionType.optionTypeFor(#{boxed_type}.class, #{default})"
      when 'string'
        type       = 'String'
        boxed_type = type
...
public class OptionsCatalog {
    ...
    <% options.each do |o| %>public static final OptionKey<<%= o.boxed_type %>> <%= o.key_constant %> = new OptionKey<>(<%= o.default_value %><%= o.option_type ? ', ' + o.option_type : '' %>);
    <% end %>
    <% options.each do |o| %>public static final OptionDescriptor <%= o.constant %> = OptionDescriptor
