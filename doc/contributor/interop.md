...
in package `org.truffleruby.interop.messages`, with an exception of `nil`. Its
messages are defined directly on the singleton object
`org.truffleruby.language.Nil`.
 
`RubyObjectMessages` defines the common behaviour for all Ruby objects. Other
Ruby core classes like `StringMessages`, `ArrayMessages`, etc. then inherit from
it and modify the behaviour.

As expected:
- `Object` implements polyglot member API.
- `Array` implements polyglot array API.
- `Proc` and `Method` have executable API.
- `String` and `Symbol` implement polyglot string API.

Any Ruby object can implement the polyglot array or member behaviour by
implementing appropriate `polyglot_*` methods. See `polyglot_object` and
`polyglot_array` subjects in detail behavior definition.

Detailed definition of the behaviour can be found in 
[another document](interop_details.md). (It is generated by
...
If any of the explicitly send messages fails then the Java `InteropException` is
translated to a Ruby error as follows:

- `UnsupportedMessageException` into `Polyglot::UnsupportedMessageError`
- `InvalidArrayIndexException`  into `IndexError`
- `UnknownIdentifierException`  into `NameError` or into `NoMethodError` if the message is `invokeMember`
- `ArityException`              into `ArgumentError`
- `UnsupportedTypeException`    into `TypeError`
